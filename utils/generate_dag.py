                ...
                if dependency_node_type == "source":
                    ...
                    if dependency_node_test_name not in dbt_sources_tasks.keys():
                        for child in manifest["child_map"][dependency_node]:
                            if "test" in child:
                                dbt_sources_tasks[dependency_node_test_name] = make_dbt_task(dependency_node_test_name, f'docker_{dependency_node_test_name}', f"source:{dependency_model_path}", True)
                                break
            dbt_tasks[node_name] = make_dbt_task(node_name, f'docker_{node_name}', model_path, False)
            for child in manifest["child_map"][node]:
                if "test" in child:
                    dbt_tasks[node_test_name] = make_dbt_task(node_test_name, f'docker_{node_test_name}', model_path, True)
                    break
    ...
    for node in manifest["nodes"].keys():
        if node.split(".")[0] == "model":
            ...
            for upstream_node in node_values["depends_on"]["nodes"]:
                ...
                elif "source" in upstream_node and any("test." in upstream_dependencies for upstream_dependencies in manifest["child_map"][upstream_node]):
...
def main(schedule):
    ...
    generated_dag_script = generate_dag_setup(file_string=generated_dag_script, project_name="jaffle-shop", schedule=schedule)
